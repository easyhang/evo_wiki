...
构建一个典雅、清爽的「数字图书馆」式 Wiki 知识库。配色采用白蓝体系：纯白背景（主背景 `#FFFFFF`、次背景 `#F8FAFC`、卡片纯白 `#FFFFFF`、浅灰描边 `#E2E8F0`）营造简洁明快的阅读体验；一个品牌强调色（示例用蓝色 `#2563EB` + 浅蓝 `#3B82F6` + 半透明光晕 `rgba(37,99,235,.12)`，可按主题替换为任意主色）用于徽标、强调与 hover 发光；标题与深色文本走黑色系（`#111111` / `#374151`）；文本走双层级（纯黑 `#111111` 正文 + 灰蓝 `#64748B` 次级）。
...
整体气质用一句话概括：以「纯白背景 + 蓝色强调 + 黑色权威标题」配中英文衬线字体，把静态内容库包装成可溯源、可探索的数字图书馆，左栏负责检索、首页用数据徽章与排行榜激发探索欲。
