...
- 新建 workspace 写入 `content_contract_version: 2`；现有配置缺少该字段时保持 v1 兼容，
  工具不会自动改写；
...
内容契约 v2 还要求每份 corpus 文件由唯一来源 Wiki 声明、来源路径规范、所有页面从首页可达。
构建报告会给出 source coverage、来源/实体映射数和歧义统计；error 阻断后续 LightRAG 写入，
warning 只提示人工处理。完整规则见[开发标准](docs/development-standard.md)。
...
- `evidence_status=partially_grounded`：存在有效证据，但部分引用或关键事实待核验；
- `evidence_status=ungrounded`：本地知识库没有可验证的覆盖（包括引用相关性不足、宽泛法条
  问题缺少本地条文支持），网关仍使用同一 LightRAG 服务的 `mode=bypass` 生成通用知识回答；
- `review_status=pending`：后台待人工审核，不折叠、不隐藏正文。

本机 `local_single_user` 预览还提供网页“审核中心”：待审核记录可展开问题、回答、对话历史与
引用，并直接通过或驳回。通过后删除受保护正文快照；驳回后保留快照。相同查询再次生成时，
响应中的 `review_history` 会报告历史驳回次数，以及新答案是否与已驳回答案完全相同。

问答页把成功展示的问答、结构化引用、当前草稿和检索参数保存在当前标签页的
`sessionStorage` 中。从来源 Wiki 返回、刷新页面或在问答/图谱间切换时会恢复原回答和
最近三轮对话上下文；“清空”会同步清除保存的问答。失败回答和审核中心详情不进入该状态，
也不会写入 `localStorage` 或服务器。
...
`ungrounded` 回答不展示这些卡片或行内可信引用，而是明确显示“暂无可信本地依据、已进入人工审核”。
引用文档的 basename 命中 `wiki-registry.json` 时，整张证据卡直接链接到对应来源 Wiki。回答正文
只会链接同时满足“已登记实体”和“本次 citation 所属来源声明了该实体 graph label”的名称；
同一实体每个回答只链接首次出现。冲突名称、无来源支持的名称以及 `李某`、`吴某` 等裸匿名名称
保持普通文本，代码、已有链接、URL、引用编号和原始 HTML 也不会被二次改写。

问答结果只展示正文、审核预警和结构化引用，不会根据引用额外请求或渲染知识子图。独立“图谱”
页面和实体页的图谱邻域继续通过 `/api/graphs` 提供图谱浏览能力；图谱加载失败不会改变回答、
证据状态或人工审核状态。
...
Nginx 文件。审核接口也只在此模式注册；trusted-proxy 模式不会暴露该接口或显示审核页签。
它是本地开发预览，不是公网服务器。
...
- 实体有图谱别名时填写可选 `graph_label` 和 `aliases`；`graph_label` 必须唯一，实体页声明的
  `sources` 会建立“引用文档 → graph label”公共映射，供引用卡来源跳转和回答正文的证据约束
  实体链接共同使用；
- 保留完整原文和用户手工编辑区；
...
| `audit resolve` | 本地写入 | `APPROVED` 结案并删除正文；`REJECTED` 结案并保留受保护快照 |
...
- [2.0 开发标准](docs/development-standard.md)
- [升级到 2.0](docs/upgrading-to-2.0.md)
